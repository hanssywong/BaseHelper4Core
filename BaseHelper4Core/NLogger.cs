...
namespace BaseHelper4Core
{
    class NLogMsg
    {
        public NLogger.LogLevel level { get; set; } = NLogger.LogLevel.Debug;
    ...
    public class NLogger
    {
        ...
        public void Init(string DirectoryPath, string LogName, LogLevel lv = LogLevel.Debug, long MaximumFileSize = 1024 * 1024 * 20)
        {
            Init(DirectoryPath, LogName, Encoding.UTF8, lv, MaximumFileSize);
        }
            ...
            if (isInitialized && isRunning)
            {
                if (IsLogLevelEnabled(level))
                {
                    try
                    {
                        ...
                        if (logmsg == null)
                        {
                            logger.Fatal(msgWithTime("Logger logmsg == null"));
                            return;
                        }
                    ...
                    {
                        logger.Fatal(ex, msgWithTime("Logger Exception"));
                    }
                }
            }
        }
        private void WriteToFile()
        {
            ...
            while (isRunning || !queue.IsQueueEmpty)
            {
                ...
                {
                    logger.Info(msgWithTime("Logger Shutdown in progress"));
                }
                catch (Exception ex)
                {
                    logger.Error(ex, msgWithTime("Logger Exception"));
                }
                finally
                {
                    if (logmsg != null)
        ...
        public void Shutdown()
        {
            if (logger != null)
            {
                ...
                logger.Info(msgWithTime("Logger Shutdown"));
                queue.Dispose();
            }
        }
    }
